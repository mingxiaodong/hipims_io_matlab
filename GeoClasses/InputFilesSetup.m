...
CaseName = 'Fuzhou15m';
CaseFolder = ['/Users/b4042552/Google Drive/Data/' CaseName '/'];
writePath = [CaseFolder 'input/field/'];
% cd(CaseFolder);
h_Or_eta = 'h';

%% ***read initial conditions from xls file
Initial_XLS = [CaseFolder CaseName '_InitialData.xlsx'];
...
NumOfHour = 72;
T_Rain = 0:3600:3600*(NumOfHour-1);
for s = 0:max(RainRecords(:,2))
    Rain = [T_Rain;RainRecords(s+1,3:end)/(3600*1000)]';
    Rain(isnan(Rain(:,2)),:)=[];    
